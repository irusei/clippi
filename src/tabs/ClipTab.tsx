        ...
        <>
          <div className="px-10 py-8 overflow-y-scroll h-full gap-y-2">
            <div className="flex flex-row gap-x-3 items-center">
              ...
              {clips.length > 0 && (
                <h2 className="text-lg font-semibold text-mocha-overlay1 mb-2">
                  {parseSize(
                    clips
                      .map((clip) => clip.size)
                      .reduce((total_size, cur_size) => total_size + cur_size),
                  )}
                </h2>
              )}
            </div>
            <div className="overflow-x-scroll w-full flex flex-row gap-x-4 py-2 mb-2">
              {uniqueGames.map(([game, count]) => (
                <div
                  key={game.name}
                  className={`p-2 h-10 flex flex-row space-x-2 bg-mocha-base rounded-lg items-center justify-center cursor-pointer border ${selectedGameName === game.name ? "border-mocha-mauve" : "border-mocha-base/50 hover:border-mocha-mauve/50"}`}
                  onClick={() =>
                    setSelectedGameName(
                      selectedGameName === game.name ? "" : game.name,
                    )
                  }
                >
